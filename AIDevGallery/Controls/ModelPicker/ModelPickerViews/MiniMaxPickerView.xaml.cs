...
internal sealed partial class MiniMaxPickerView : BaseModelPickerView
{
    private ObservableCollection<ModelDetails> models = new ObservableCollection<ModelDetails>();

    public MiniMaxPickerView()
    {
    ...
    private async Task Load()
    {
        ...
        var miniMaxModels = await MiniMaxModelProvider.Instance.GetModelsAsync();

        if (miniMaxModels == null || !miniMaxModels.Any())
        {
            VisualStateManager.GoToState(this, "ShowInput", true);
        }
        else
        {
            miniMaxModels.ToList().ForEach(models.Add);
    ...
    {
        if (string.IsNullOrEmpty(MiniMaxKeyTextBox.Text))
        {
            return;
        }

        MiniMaxModelProvider.MiniMaxKey = MiniMaxKeyTextBox.Text;
        _ = Load();
    }
    ...
    private void RemoveKeyButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        MiniMaxModelProvider.Instance.ClearCachedModels();
        _ = Load();
    }
}
