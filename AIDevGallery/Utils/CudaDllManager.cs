...
{
    // Official Microsoft OnnxRuntimeGenAI NuGet package v0.10.1
    // This downloads the entire NuGet package, then extracts the CUDA DLL
    private const string NuGetPackageUrl = "https://www.nuget.org/api/v2/package/Microsoft.ML.OnnxRuntimeGenAI.Cuda/0.10.1";
    ...
    public static async Task<bool> EnsureCudaDllAsync(IProgress<float>? progress = null, bool forceRetry = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                using (var archive = ZipFile.OpenRead(tempNupkgPath))
                {
