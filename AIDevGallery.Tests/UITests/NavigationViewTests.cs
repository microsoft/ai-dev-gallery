...
public class NavigationViewTests : FlaUITestBase
{
    ...
    [Description(@"Performance test measuring page load time from first sample item navigation to model selection UI availability.
    
    Background: When users navigate to first sample item, the app shows a loading state before displaying the 'Selected models' button. 
    This delay impacts user experience and needs monitoring for performance regressions.

    Goal: Establish baseline performance metric (TimeToSelectedModelsButtonMs) for CI/CD pipeline monitoring and regression detection.
    Typical values: 2-8 seconds. Alert threshold: >10 seconds indicates performance degradation.")] 
    public void NavigationView_Perf_TimeToModelSelectionButton()
    {
        Assert.IsNotNull(MainWindow, "Main window should be initialized");

        var menuItemsHost = MainWindow.FindFirstDescendant(cf => cf.ByAutomationId("MenuItemsHost"));
        Assert.IsNotNull(menuItemsHost, "MenuItemsHost should be found");
        var topLevelItems = menuItemsHost.FindAllChildren(cf => cf.ByControlType(ControlType.ListItem));
        ...
        var textItem = MainWindow.FindFirstDescendant(cf => cf.ByControlType(ControlType.ListItem).And(cf.ByName("Text")));
        Assert.IsNotNull(textItem, "Text list item should be found");
        textItem.Click();
        Thread.Sleep(1000);

        // Get the first sample item under the Text category
        var textItemChildren = textItem.FindAllChildren(cf => cf.ByControlType(ControlType.ListItem));
        AutomationElement? firstSampleItem = null;
        ...
        if (textItemChildren.Length == 0)
        {
            ...
            if (innerNavView != null)
            {
                ...
                {
                    var allListItems = innerMenuHost.FindAllDescendants(cf => cf.ByControlType(ControlType.ListItem));
                    var categoryItems = innerMenuHost.FindAllChildren(cf => cf.ByControlType(ControlType.ListItem));
        ...
        try
        {
            ...
            if (!eventTriggered)
            {
                ...
                for (int i = 0; i < 150; i++)
                {
                    ...
                    if (button == null)
                    {
                        button = MainWindow.FindAllDescendants(cf => cf.ByControlType(ControlType.Button))
            ...
            Console.WriteLine($"Perf: TimeToSelectedModelsButtonMs={elapsedMs}");
            TestContext?.WriteLine($"PerfMetric TimeToSelectedModelsButtonMs {elapsedMs}");
